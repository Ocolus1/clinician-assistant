...
export default function PatientBudgetTab({ 
  ...
  const { 
    ...
    queryFn: async () => {
      console.log(`Fetching budget settings for patient ID: ${patientId} with ALL=true`);
      try {
  ...
  // Fetch budget items for the patient - ensure we get the latest usage data
  const { 
    data: budgetItems = [], 
  ...
  // Fetch sessions for the patient to calculate actual used funds
  const { 
  ...
  const createPlanMutation = useMutation({
    mutationFn: async (newPlan: any) => {
      ...
      try {
        ...
        if (validPlanData.isActive === true) {
          ...
          // Get all budget settings for this patient that are active
          ...
          for (const activePlan of activeSettings) {
            ...
            const deactivateResult = await fetch(`/api/budget-settings/${activePlan.id}`, {
              ...
              body: JSON.stringify({ 
                ...
                availableFunds: Number(activePlan.ndisFunds),
        ...
        const response = await apiRequest('POST', `/api/patients/${patientId}/budget-settings`, validPlanData);
        ...
        if (newItems && newItems.length > 0) {
          ...
          for (const item of itemsWithIds) {
            const itemResponse = await apiRequest('POST', `/api/patients/${patientId}/budget-items`, item);
            if (!itemResponse.ok) {
          ...
          queryClient.invalidateQueries({ queryKey: ['/api/patients', patientId, 'budget-items'] });
        }
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/patients', patientId, 'budget-settings'] });
      toast({
        title: "Success",
    ...
    try {
      ...
      if (activeSettings.length > 0) {
        ...
        for (const activePlan of activeSettings) {
          ...
          try {
            ...
            const deactivateData = {
              ...
              availableFunds: Number(activePlan.ndisFunds),
              endOfPlan: activePlan.endOfPlan || null
              // Remove patientId as it should be omitted according to the schema
            };
        ...
        const activateData = {
          ...
          // Remove patientId to match schema requirements
        };
