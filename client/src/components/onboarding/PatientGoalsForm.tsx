...
interface PatientGoalsFormProps {
  patientId: number;
  onComplete: () => void;
  onPrevious: () => void;
}

export default function PatientGoalsForm({ patientId, onComplete, onPrevious }: PatientGoalsFormProps) {
  ...
  const { data: goals = [] } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/patients/${patientId}/goals`);
      return res.json();
    }
  });
  ...
  const createGoal = useMutation({
    mutationFn: async (data: any) => {
      const res = await apiRequest("POST", `/api/patients/${patientId}/goals`, data);
      return res.json();
    },
    onSuccess: (data) => {
      goalForm.reset();
      ...
      <div className="space-y-6">
          ...
          <DialogContent>
            ...
            <Form {...goalForm}>
              <form onSubmit={goalForm.handleSubmit((data) => {
                ...
                <div className="flex gap-2 justify-end">
                  <Button variant="outline" onClick={() => setShowGoalEditDialog(false)}>
                    Cancel
                  </Button>
                  <Button 
                    type="submit" 
          ...
          <DialogContent>
            ...
            <Form {...subgoalForm}>
              <form onSubmit={subgoalForm.handleSubmit((data) => {
                ...
                <div className="flex gap-2 justify-end">
                  <Button variant="outline" onClick={() => setShowSubgoalEditDialog(false)}>
                    Cancel
                  </Button>
                  <Button 
                    type="submit" 
        ...
        {!canProceed && validationMessage && (
          <Alert className="mt-4">
            <AlertTitle>Cannot proceed yet</AlertTitle>
