...
const ClinicianAssistantTile: React.FC<ClinicianAssistantTileProps> = ({ 
  ...
  const handleNavigate = () => {
    setLocation("/clinician-assistant");
  };
  
  return (
    <Card onClick={handleNavigate} className={cn("overflow-hidden w-64 transition-all hover:shadow-md py-6 cursor-pointer group hover:shadow-md cursor-pointer hover:translate-y-[-2px]", className)}
