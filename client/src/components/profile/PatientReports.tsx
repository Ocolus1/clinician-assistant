...
// API and Utils
import { getPatientPerformanceReport, getPatientStrategiesReport, type PatientReportData, type StrategiesData } from "@/lib/api/patientReports";
...
export function PatientReports({ patientId }: PatientReportsProps) {
  ...
  const reportQuery = useQuery({
    queryKey: ['/api/patients/reports/performance', patientId, dateRangeParams],
    queryFn: getPatientPerformanceReport,
  ...
  const strategiesQuery = useQuery({
    queryKey: ['/api/patients/reports/strategies', patientId, dateRangeParams],
    queryFn: getPatientStrategiesReport,
    refetchOnWindowFocus: false,
    enabled: !!dateRangeParams?.startDate,
  });
  ...
  useEffect(() => {
    if (patientId) {
      const fetchBudgetSummary = async () => {
        try {
          console.log("Fetching budget summary for patient:", patientId);
          
          // First, let's check what API endpoints are returning
          const budgetItems = await budgetUtilizationService.fetchBudgetItems(patientId);
          console.log("Budget items:", budgetItems);
          
          const budgetSettings = await budgetUtilizationService.fetchBudgetSettings(patientId);
          console.log("Budget settings:", budgetSettings);
          
          const sessions = await budgetUtilizationService.fetchPatientSessions(patientId);
          console.log("Patient sessions:", sessions);
          
          // Now get the full budget summary
          const summary = await budgetUtilizationService.getBudgetSummary(patientId);
  ...
  useEffect(() => {
    // Make sure we have a patient ID and a valid budget start date
    if (patientId && selectedBudgetPlan && selectedBudgetPlan.createdAt) {
      const fetchGoalData = async () => {
        try {
          console.log("Fetching goal performance data for patient:", patientId);
          ...
          // Fetch goal performance data - convert patientId to string
          const goals = await fetchGoalPerformanceData(String(patientId), budgetStartDate, budgetEndDate);
  ...
  if (isLoadingBudgetSettings || reportQuery.isLoading || strategiesQuery.isLoading) {
    return (
      <div className="py-12 flex justify-center items-center">
        ...
        <span className="ml-4 font-medium text-base text-muted-foreground">Loading patient performance data...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 max-w-screen px-1 py-2">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold">Patient Performance Reports</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <CardContent className="p-4 pt-0 text-center">
            <p className="text-sm text-slate-600">
              View all professionals involved in patient treatment
            </p>
            {reportData?.patientDetails?.caregivers && reportData.patientDetails.caregivers.length > 0 ? (
              <div className="mt-4 text-sm font-medium">
                {reportData.patientDetails.caregivers.length} team members
              </div>
            ) : null}
          </CardContent>
        </Card>
      ...
      <EnhancedBudgetUtilizationModal
        ...
        patientId={patientId}
        budgetSummary={null} // Fetch data within the component
      ...
      <ReportModal
        ...
        onClose={closeModal}
        title="Observations"
        description="Tracking patient behaviors and responses during sessions"
        detailsContent={
      ...
      <ReportModal
        ...
        description="Overview of all professionals working with this patient"
      >
        <div className="text-center py-12 text-muted-foreground">
...
function ObservationsSection({ data }: { data?: PatientReportData }) {
  if (!data) return null;
...
function PatientInfoCard({ data, className }: { 
  data?: PatientReportData;
  className?: string;
}) {
  if (!data) return null;
  ...
  return (
    <Card className={cn(
      ...
      <CardHeader className="py-4 px-5">
        <CardTitle className="text-base font-semibold text-gray-800">Patient Details</CardTitle>
      </CardHeader>
      <CardContent className="p-5">
        ...
        <div className="h-[170px] flex flex-col justify-between py-2">
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold text-sm text-gray-900">{patientDetails.name}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600 text-sm font-medium">Age:</span>
              <span className="font-semibold text-sm text-gray-900">{patientDetails.age} years</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600 text-sm font-medium">Funds Management:</span>
              <span className="font-semibold text-sm text-gray-900">{patientDetails.fundsManagement}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600 text-sm font-medium">Allied Health Team:</span>
              <span className="font-semibold text-sm text-gray-900">{patientDetails.caregivers.length}</span>
            </div>
          </div>
        </div>
      </CardContent>
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {strategy.description || 'Therapeutic strategy used to support patient goals and outcomes.'}
        </p>
      </div>
...
function ObservationScoresOverview({ data }: { data?: PatientReportData }) {
  if (!data) return null;
...
function ObservationHistory({ data }: { data?: PatientReportData }) {
  if (!data) return null;
  ...
  return (
    <div className="space-y-6">
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <RechartsTooltip 
              ...
              formatter={(value: number) => [`${value.toFixed(1)}/10`, getMetricDisplayName(selectedMetric)]}
            ...
            <Line
              ...
              strokeWidth={3}
              dot={{ r: 6, strokeWidth: 2, fill: 'white', stroke: getMetricColor(selectedMetric) }}
              activeDot={{ r: 8, strokeWidth: 2, fill: 'white', stroke: getMetricColor(selectedMetric) }}
            />
          </RechartsLineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mt-4">
        <Card>
          <CardHeader>
            <CardTitle>Observation Insights</CardTitle>
            <CardDescription>Analysis of current trends</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-center py-8">
              <p className="text-muted-foreground mb-4">
                AI-powered observation insights coming soon.
              </p>
            </div>
          </CardContent>
        </Card>
...
function GoalsSection({ data }: { data?: PatientReportData }) {
  ...
  // Define a type for patient goals
  ...
  const { data: subgoals, isLoading: isLoadingSubgoals } = useQuery<Subgoal[]>({
    queryKey: ['/api/patients/goals', selectedGoal?.id, 'subgoals'],
    queryFn: ({ queryKey }) => {
      return apiRequest('GET', `/api/patients/goals/${queryKey[1]}/subgoals`);
    },
    enabled: !!selectedGoal?.id,
  });
  
  // Fetch all goals for the patient
  const { data: allGoals, isLoading: isLoadingGoals } = useQuery<PatientGoal[]>({
    queryKey: ['/api/patients', data?.patientDetails.id, 'goals'],
    queryFn: ({ queryKey }) => {
      return apiRequest('GET', `/api/patients/${queryKey[1]}/goals`);
    },
    enabled: !!data?.patientDetails.id,
  });
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Track progress on treatment goals</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  const handleGoalClick = (goal: any) => {
    // If the same goal is clicked again, toggle the details panel
    if (selectedGoal && selectedGoal.id === goal.id) {
      setShowGoalDetails(!showGoalDetails);
      return;
    }
    
    // Set the selected goal and show details
    setSelectedGoal(goal);
    setShowGoalDetails(true);
    
    // Find the corresponding full goal data from allGoals
    const fullGoal = allGoals?.find(g => g.id === goal.id);
    
    // If we have the full goal data, update the selected goal with it
    if (fullGoal) {
      setSelectedGoal({
        ...goal,
      ...
      <Card>
        ...
        <CardContent>
          {goals.length === 0 ? (
            <div className="flex items-center justify-center h-40">
              <AlertCircle className="h-6 w-6 text-muted-foreground mr-2" />
              <p className="text-muted-foreground">No goals have been set</p>
            </div>
          ) : (
            <div className="space-y-6">
                      ...
                      <div className="mt-4 pt-4 border-t text-sm">
                        {isLoadingSubgoals ? (
                          <div className="flex items-center justify-center py-4">
                            ...
                            <span className="text-xs text-muted-foreground">Loading subgoals...</span>
                          </div>
                        ) : subgoals && subgoals.length > 0 ? (
                          <div className="space-y-3">
                            <h5 className="font-medium text-xs text-muted-foreground mb-2">SUBGOALS</h5>
                            {subgoals.map((subgoal) => (
                              <div key={subgoal.id} className="flex items-start space-x-2">
                                <div className="h-4 w-4 mt-0.5 rounded-full bg-primary/20 flex items-center justify-center">
                                  <div className="h-2 w-2 rounded-full bg-primary"></div>
                                </div>
                                <p className="text-xs flex-1">{subgoal.title}</p>
                              </div>
                            ))}
                          </div>
                        ) : (
                          <p className="text-xs text-muted-foreground">No subgoals found for this goal.</p>
                        )}
                      </div>
                    )}
                  </div>
...
function GoalVerticalBar({ score }: { score: number }) {
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="relative h-12 w-4 bg-muted rounded-full overflow-hidden">
            <div 
        ...
        <TooltipContent side="top">
          <div className="text-xs">
            <div className="font-medium mb-1">Goal Score: {score.toFixed(1)}/10</div>
            <div className="text-muted-foreground">
              {score >= 7 ? "Excellent progress" : 
               score >= 4 ? "Good progress" : 
               "Needs attention"}
            </div>
          </div>
        </TooltipContent>
