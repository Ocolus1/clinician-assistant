...
const newCaregiverSchema = z.object({
...
// Form values type
type NewCaregiverFormValues = z.infer<typeof newCaregiverSchema>;
...
export default function AddCaregiverDialog({
  ...
  // Initialize form
  const form = useForm<NewCaregiverFormValues>({
    resolver: zodResolver(newCaregiverSchema),
    defaultValues: {
      name: "",
      email: "",
      relationship: "",
  ...
  // Create caregiver mutation
  const createCaregiverMutation = useMutation({
    mutationFn: (data: NewCaregiverFormValues) => {
      return apiRequest('POST', `/api/patients/${patientId}/caregivers`, data);
    },
    onSuccess: () => {
      onOpenChange(false);
      form.reset();
      ...
      toast({
        title: "Success",
        description: "New caregiver created successfully!",
      });
    },
    onError: (error) => {
  ...
  const onSubmit = (data: NewCaregiverFormValues) => {
    ...
    >
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new caregiver for {patientName}'s support network.
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea 
                      {...field} 
                      placeholder="Additional notes about this caregiver (optional)"
            ...
            <DialogFooter>
              ...
              >
                {createCaregiverMutation.isPending ? "Creating..." : "Create Caregiver"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
