...
import { Session, Patient } from "@shared/schema";
...
export default function PatientSessions({ patientId: propPatientId }: PatientSessionsProps) {
  const params = useParams();
  // Use the prop patientId if provided, otherwise try to get it from the URL
  const patientId = propPatientId || (params.id ? parseInt(params.id) : undefined);
  ...
  const { data: patient } = useQuery<Patient>({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        throw new Error(`Error fetching patient: ${response.status} ${response.statusText}`);
      }
  ...
  // Fetch patient goals to use for populating titles in assessments
  ...
  const goalTitlesMap = React.useMemo(() => {
    if (!patientGoals) return {};
    return patientGoals.reduce((acc: Record<number, string>, goal: any) => {
      acc[goal.id] = goal.title;
      return acc;
    }, {});
  }, [patientGoals]);
  
  // Fetch subgoals for all patient goals
  const { data: subgoalsData } = useQuery({
    queryKey: ['/api/patients', patientId, 'all-subgoals'],
    queryFn: async () => {
      if (!patientGoals || !patientGoals.length) return {};
      
      // Create a map of goalId to array of subgoals
      const subgoalsMap: Record<number, any[]> = {};
      
      // Fetch subgoals for each goal
      await Promise.all(patientGoals.map(async (goal: any) => {
    ...
    enabled: !!patientGoals && patientGoals.length > 0
  });
  ...
  const prepareSessionFormData = (): NewSessionFormValues | undefined => {
    ...
    const sessionNote = {
      presentCaregivers: sessionNotesData?.presentCaregivers || [],
      presentCaregiverIds: sessionNotesData?.presentCaregiverIds || [],
    ...
    const goalAssessments = sessionAssessmentsData 
      ? Object.values(
    ...
    console.log("Transformed goal assessments for form:", goalAssessments);
