/**
 * Comprehensive Chatbot Test Runner (TypeScript Version)
 * 
 * This script runs all the chatbot TypeScript test files and reports the results.
...
const databaseIndependentTests = [
  'entity-extraction-enhanced-test.ts'
];
...
const reportFile = path.join(__dirname, '..', `test-report-ts-${timestamp}.txt`);
...
async function runAllTests() {
  console.log(chalk.blue('===== Clinician Chatbot Test Suite (TypeScript) =====\n'));
  ...
  // Save test report
  const reportContent = `
Clinician Chatbot Test Report (TypeScript)
Date: ${new Date().toISOString()}

Database-Independent Tests:
${databaseIndependentTests.map(test => `- ${test}: ${databaseIndependentTests.includes(test) ? 'Passed' : 'Failed'}`).join('\n')}

Database-Dependent Tests:
${databaseDependentTests.map(test => `- ${test}: ${databaseDependentTests.includes(test) ? 'Passed' : 'Failed'}`).join('\n')}
