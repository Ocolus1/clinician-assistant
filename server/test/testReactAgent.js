...
const getPatientName = () => {
  return new Promise((resolve, reject) => {
    ...
    const command = `node -e "
      ...
      const pool = new Pool({
        connectionString: 'postgresql://neondb_owner:npg_LUnslq07MhAw@ep-soft-fire-a59uc832.us-east-2.aws.neon.tech/neondb?sslmode=require'
      });
...
const prepareQuestions = async () => {
  try {
    ...
    const preparedQuestions = testQuestions.map(q => ({
      ...
      expectedKeywords: q.expectedKeywords.map(k => k === 'PATIENT_NAME' ? patientName : k)
    }));
    
    fs.writeFileSync(testQuestionsFile, JSON.stringify(preparedQuestions, null, 2));
  ...
  return new Promise((resolve, reject) => {
    const command = `node -e "
      import('./dist/server/services/reactAgentService.js').then(async ({ reactAgentService }) => {
        try {
          const answer = await reactAgentService.processQuery('${question.replace(/'/g, "\\'")}', 999);
